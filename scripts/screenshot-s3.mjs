// Screenshot delle schermate S3 (tema chiaro/scuro, note guida, presa del comando)
// con Chrome headless e un server di prova → docs/screenshots/s3/
// Uso: node scripts/screenshot-s3.mjs   (serve la build in dist/)
...
const PORTA_TEST = 4997;
const BASE = `http://127.0.0.1:${PORTA_TEST}`;
const CDP_PORT = 9335;
const CHROME = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const CARTELLA = "docs/screenshots/s3";
...
const dati = fs.mkdtempSync(path.join(os.tmpdir(), "regia-shot3-"));
...
fs.rmSync("/tmp/regia-shot3-chrome", { recursive: true, force: true });
const chrome = spawn(
  CHROME,
  [
    "--headless=new",
    `--remote-debugging-port=${CDP_PORT}`,
    "--user-data-dir=/tmp/regia-shot3-chrome",
    "--no-first-run",
    "--mute-audio",
...
await attendi(1000);
const banner = await mac.js(`document.body.innerText.includes('ha preso il comando')`);
