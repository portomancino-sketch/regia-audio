// Screenshot S8-bis: riga Sempre DENTRO il vetro del dock (telefono 390×844,
// 1 e 5 pillole, in cima e in fondo, chiaro/scuro), Mac Live, card che suona
// col Sfuma/Stop grandi. Chrome headless + server di prova → docs/screenshots/s8bis/
// Uso: node scripts/screenshot-s8bis.mjs   (serve la build in dist/)
...
const CARTELLA = "docs/screenshots/s8bis";
...
const dati = fs.mkdtempSync(path.join(os.tmpdir(), "regia-shot8bis-"));
...
fs.rmSync("/tmp/regia-shot8bis-chrome", { recursive: true, force: true });
const chrome = spawn(
  CHROME,
  [
    "--headless=new",
    `--remote-debugging-port=${CDP_PORT}`,
    "--user-data-dir=/tmp/regia-shot8bis-chrome",
    "--no-first-run",
    "--mute-audio",
...
await tel.finoA(`document.querySelector('[aria-label="Sempre"] button[aria-pressed="true"]') !== null`);
await attendi(400);
await scattaCimaEFondo(tel, "01-telefono-1-pillola-chiaro");
await tel.tema("scuro");
await tel.click("Ok, pronti");
await attendi(400);
await scattaCimaEFondo(tel, "02-telefono-1-pillola-scuro");

// ---- Cinque caselle Sempre ----
...
if (!cinque) problemi++;
await tel.clickPillola("Luci giù");
await attendi(400);
await scattaCimaEFondo(tel, "03-telefono-5-pillole-scuro");
await tel.tema("chiaro");
await tel.click("Ok, pronti");
await attendi(400);
await scattaCimaEFondo(tel, "04-telefono-5-pillole-chiaro");

// ---- Controlli: striscia dentro il vetro e opaca; ultima card sopra il dock ----
const cs = await tel.js(CONTROLLO_STRISCIA);
console.log(`${cs.ok ? "  ✓" : "  ✗"} striscia Sempre dentro il vetro del dock e opaca (dentro=${cs.dentro}, punti bucati=${cs.bucati}, card geometricamente sotto la striscia da non scrollato=${cs.sovrapposte})`);
if (!cs.ok) problemi++;
...
const misure = await tel.js(`(() => {
  ...
  const s = document.querySelector('[aria-label="Sempre"]');
  return { h: ps.map(p => Math.round(p.height)), wMax: Math.round(Math.max(...ps.map(p => p.width))), scorre: s.scrollWidth > s.clientWidth, scrollbar: getComputedStyle(s).scrollbarWidth };
})()`);
const misureOk = misure.h.every((h) => h === 44) && misure.wMax <= 390 * 0.6 && misure.scorre;
console.log(`${misureOk ? "  ✓" : "  ✗"} pillole 44px, larghezza max ${misure.wMax}px, striscia scorre=${misure.scorre}, scrollbar=${misure.scrollbar}`);
if (!misureOk) problemi++;

// ---- Mac: Live con la riga Sempre dentro il vetro ----
await mac.js(`window.scrollTo(0, 0)`);
await attendi(300);
await mac.scatta("05-mac-live-sempre-chiaro");
const csMac = await mac.js(CONTROLLO_STRISCIA);
console.log(`${csMac.ok ? "  ✓" : "  ✗"} Mac: riga Sempre dentro il vetro del dock (dentro=${csMac.dentro}, punti bucati=${csMac.bucati})`);
if (!csMac.ok) problemi++;

// ---- Card che suona sul telefono: Sfuma e Stop grandi ----
await tel.click("STOP TUTTO");
await attendi(400);
await tel.clickCue("Treno in corsa");
const cardSuona = await tel.finoA(`[...document.querySelectorAll('.grid > [role=button] button')].some(b => b.textContent.trim() === 'Sfuma')`);
console.log(cardSuona ? "  ✓ card che suona: tasti Sfuma / Stop grandi" : "  ✗ tasti grandi assenti");
if (!cardSuona) problemi++;
const tasti = await tel.js(`(() => {
  const card = [...document.querySelectorAll('.grid > [role=button]')].find(c => c.textContent.includes('Treno in corsa'));
  const bs = [...card.querySelectorAll('button')].map(b => { const r = b.getBoundingClientRect(); return { t: b.textContent.trim(), h: Math.round(r.height), w: Math.round(r.width), l: Math.round(r.left), r: Math.round(r.right) }; });
  return bs;
})()`);
const tastiOk = tasti.length === 2 && tasti.every((b) => b.h === 52) && Math.abs(tasti[0].w - tasti[1].w) <= 1 && tasti[1].l - tasti[0].r === 10;
console.log(`${tastiOk ? "  ✓" : "  ✗"} tasti: ${tasti.map((b) => `${b.t} ${b.w}×${b.h}`).join(", ")}, spazio ${tasti[1]?.l - tasti[0]?.r}px`);
if (!tastiOk) problemi++;
await tel.js(`window.scrollTo(0, 0)`);
await attendi(300);
await tel.scatta("06-telefono-card-suona-chiaro");
await tel.tema("scuro");
await tel.click("Ok, pronti");
await attendi(500);
await tel.scatta("07-telefono-card-suona-scuro");
// Stop dalla card: ferma davvero
await tel.js(`(() => { const b = [...document.querySelectorAll('.grid > [role=button] button[aria-label="Ferma subito"]')][0]; b?.click(); })()`);
const fermato = await tel.finoA(`!document.body.innerText.includes('finisce tra')`, 4000);
console.log(fermato ? "  ✓ 'Stop' sulla card ferma il suono" : "  ✗ Stop sulla card");
if (!fermato) problemi++;
